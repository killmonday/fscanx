...
func TestScanByChan(t *testing.T) {
	...
	//			}
	//			CallScanTaskByProtocol("http", &targetInfo, &wg2)
	//
	...
	if len(ipLists) > 0 || len(common.HostAndPortList) > 0 {
		...
		for _, portInfoOfAlive := range alivePortInfoSlice {
			...
			case targetInfo.Ports == "445":
				CallScanTaskByPort(ms17010, &targetInfo, &wg)          // ms17010
				CallScanTaskByPort(targetInfo.Ports, &targetInfo, &wg) // smb
				CallScanTaskByPort("1000002", &targetInfo, &wg)        // smbghost
			case targetInfo.Ports == "9000":
				CallScanTaskByPort(targetInfo.Ports, &targetInfo, &wg) // fcgiscan
			...
			default:
				...
				if common.UseNmap {
					if PluginListByProto[protocol] != nil {
						...
						CallScanTaskByPort(web, &targetInfo, &wg) // plugins scan
					}
				} else {
					// 这里是插件未覆盖的协议，那么只进行http扫描识别就行
					CallScanTaskByPort(web, &targetInfo, &wg) //webtitle
				}
