...
2026.08.13，README 补充带认证 socks5 代理的用法说明（socks5://用户名:密码@ip:端口），详见第 9 节使用案例（5）。此前代码已支持账号密码认证，但文档一直没写，容易误以为不支持。
...
如果 socks5 服务端需要账号密码认证（自建的 gost、3proxy 等工具常开启认证），把账号密码写进地址即可：

fscanx  -socks5  socks5://user:pass@127.0.0.1:1080   -h  192.168.1.1/16  -auto

格式为 socks5://用户名:密码@ip:端口，端口扫描、协议识别、web扫描、poc、爆破均支持带认证的 socks5。密码里含有 @、: 等特殊字符时一般可以直接写，如果遇到解析问题再用 URL 转义写法（@ 写成 %40，: 写成 %3A）。
